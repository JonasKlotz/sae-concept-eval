...
class DictionaryLayer(nn.Module):
    ...
    def __init__(
        ...
        self._weights = nn.Parameter(
            torch.empty(nb_concepts, in_dimensions, device=device)
        )
        if initializer is None:
            nn.init.xavier_uniform_(self._weights)
        else:
            assert initializer.shape == self._weights.shape, (
                f"Invalid initializer {initializer.shape}, must have the same shape as the dictionary {self._weights.shape}. "
            )
            self._weights.data = torch.tensor(
                initializer.clone().detach(), device=device
            )
