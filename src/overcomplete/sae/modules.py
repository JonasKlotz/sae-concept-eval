...
class MLPEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        assert len(x.shape) == 2, (
            "Input tensor must have 2 dimensions (batch, input_size)."
        )
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        assert len(dims) == 2, (
            "The input dimensions must be a tuple of (nb_token, nb_dim)."
        )
        ...
        self.attn = nn.MultiheadAttention(
            self.embed_dim, self.num_heads, dropout=attn_drop
        ).to(device)
    ...
    def forward(self, x):
        ...
        assert len(x.shape) == 3, (
            "Input tensor must have 3 dimensions (batch, seq_length, input_size)."
        )
...
class AttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        assert len(input_shape) == 2, (
            "The input shape must be a tuple of (nb_token, nb_dim)."
        )
...
class ResNetBlock(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, stride=1, activation=nn.GELU, device="cpu"
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        assert len(x.shape) == 4, (
            "Input tensor must have 4 dimensions (batch, channels, height, width)."
        )
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(
        ...
        assert len(input_shape) == 3, (
            "The input shape must contain 3 dimensions (channels, height, width)."
        )
