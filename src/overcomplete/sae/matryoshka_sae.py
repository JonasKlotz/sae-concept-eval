...
class BaseAutoencoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.nb_concepts = self.config["dict_size"]
        self.b_dec = nn.Parameter(torch.zeros(self.config["input_shape"]))
        self.b_enc = nn.Parameter(torch.zeros(self.config["dict_size"]))
        self.W_enc = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(self.config["input_shape"], self.config["dict_size"])
            )
        )
        self.W_dec = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(self.config["dict_size"], self.config["input_shape"])
            )
        )
...
class GlobalBatchTopKMatryoshkaSAE(BaseAutoencoder):
    def __init__(self, cfg):
        ...
        self.nb_concepts = total_dict_size

        self.group_sizes = cfg["group_sizes"]

        self.group_indices = [0] + list(
            torch.cumsum(torch.tensor(cfg["group_sizes"]), dim=0)
        )
        ...
        self.W_enc = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(cfg["input_shape"], total_dict_size)
            )
        )

        self.W_dec = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(total_dict_size, cfg["input_shape"])
            )
        )
    ...
    def compute_activations(self, x_cent):
        ...
        if self.training:
            acts_topk = torch.topk(
                acts.flatten(), self.config["top_k"] * x_cent.shape[0], dim=-1
            )
            acts_topk = (
    ...
    def encode(self, x):
        ...
        acts_topk[:, max_act_index:] = 0
        if len(original_shape) == 3:
            acts_topk = acts_topk.reshape(original_shape[0], original_shape[1], -1)
        return acts_topk, acts
    ...
    def get_loss_dict(
        ...
        for intermediate_reconstruct in intermediate_reconstructs:
            l2_losses = torch.cat(
                [
                    l2_losses,
                    (intermediate_reconstruct.float() - x.float())
                    .pow(2)
                    .mean()
                    .unsqueeze(0),
                ]
            ...
            total_l2_loss += (
                (intermediate_reconstruct.float() - x.float()).pow(2).mean()
            )
    ...
    def get_auxiliary_loss(self, x, x_reconstruct, all_acts):
        ...
        if aux_reconstruct.abs().sum() > 0:
            aux_loss = (
                ...
                * (aux_reconstruct.float() - residual.float()).pow(2).mean()
            )
            return aux_loss
...
class JumpReLUSAE(BaseAutoencoder):
    ...
    def encode(self, x):
        ...
        return feature_magnitudes, pre_activations
    ...
    def get_loss_dict(self, x, x_reconstruct, acts, x_mean, x_std):
        ...
        l0 = (
            StepFunction.apply(
                acts, self.jumprelu.log_threshold, self.config["bandwidth"]
            )
            .sum(dim=-1)
            .mean()
        )
...
def resolve_topk(sparsity, nb_concepts: int) -> int:
    """
    Convert a sparsity specification into an integer k.

    Parameters
    ----------
    sparsity : float or int
        - float in (0, 1]  → fraction of active units
        - int   >= 1       → exact number of active units
        - 1 (float or int) → no sparsity if nb_concepts == 1, otherwise k = 1
    nb_concepts : int
        Total number of dictionary elements.

    Returns
    -------
    k : int
        Number of active units to keep (1 ≤ k ≤ nb_concepts).
    """
    if isinstance(sparsity, float):
        if not (0.0 < sparsity <= 1.0):
            raise ValueError("Float sparsity must be in (0, 1].")
        k = int(round(sparsity * nb_concepts))
        return max(1, min(k, nb_concepts))

    if isinstance(sparsity, int):
        if sparsity < 1:
            raise ValueError("Integer sparsity must be >= 1.")
        return min(sparsity, nb_concepts)

    raise TypeError("sparsity must be float or int.")


class RandomSAE(BaseAutoencoder):
    """
    SAE baseline that ignores the input and returns random codes.

    Sparsity is fixed at initialization via cfg["sparsity"].

    - sparsity = 1        -> no sparsity (keep all units)
    - sparsity in (0,1]  -> keep that fraction of units (top-|.| per sample)
    - sparsity >= 1 (int)-> keep exactly k units
    """

    def __init__(self, cfg):
        super().__init__(cfg)

        self.sparsity = cfg.get("top_k", 1)

        if "seed" in self.config and self.config["seed"] is not None:
            self._gen = torch.Generator(device=self.config["device"])
            self._gen.manual_seed(int(self.config["seed"]))
        else:
            self._gen = None
    ...
    def encode(self, x: torch.Tensor):
        ...
        ----------
        x : torch.Tensor
            Input tensor, used only to infer batch size, device, and dtype.

        Returns
        -------
        code : torch.Tensor
            Random dense code, shape [B, dict_size].
        sparse_code : torch.Tensor
            Sparsified version of code according to self.sparsity.
        """
        B = x.shape[0]
        device = x.device
        dtype = x.dtype
        ...
        if self._gen is None:
            code = torch.randn(B, self.nb_concepts, device=device, dtype=dtype)
        else:
            code = torch.randn(
                B, self.nb_concepts, device=device, dtype=dtype, generator=self._gen
            )
        ...
        k = resolve_topk(self.sparsity, self.nb_concepts)
        idx = code.abs().topk(k, dim=-1, largest=True).indices
        sparse_code = torch.zeros_like(code).scatter_(-1, idx, code.gather(-1, idx))
        return code, sparse_code

    def decode(self, sparse_code: torch.Tensor):
        """
        Decode the sparse code into the input space.
        """
        recon = sparse_code @ self.W_dec + self.b_dec
        return recon

    def forward(self, x: torch.Tensor):
        """
        Returns a dummy reconstruction for compatibility with SAE pipelines.
        """
        code, sparse_code = self.encode(x)
        recon = sparse_code @ self.W_dec + self.b_dec
        return recon, code, sparse_code


class FrozenEncoderSAE(BaseAutoencoder):
    def __init__(self, cfg):
        super().__init__(cfg)

        self.sparsity = cfg.get("top_k", 1)

        # Freeze encoder params that are already created in BaseAutoencoder
        self.W_enc.requires_grad_(False)
        self.b_enc.requires_grad_(False)

    @torch.no_grad()
    def encode(self, x: torch.Tensor):
        code = x @ self.W_enc + self.b_enc
        ...
        idx = code.abs().topk(k, dim=-1).indices
        sparse_code = torch.zeros_like(code).scatter_(-1, idx, code.gather(-1, idx))
        return code, sparse_code

    def decode(self, sparse_code: torch.Tensor):
        return sparse_code @ self.W_dec + self.b_dec

    def forward(self, x: torch.Tensor):
        code, sparse_code = self.encode(x)
        recon = self.decode(sparse_code)
        return recon, code, sparse_code
