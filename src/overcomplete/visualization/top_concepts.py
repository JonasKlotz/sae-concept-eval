...
def contour_top_image(
    images, heatmaps, concept_id, percentiles=None, cmap="viridis", linewidth=1.0
):
    """
    ...
    for i, idx in enumerate(best_ids):
        ...
        for percentile in percentiles:
            if len(percentiles) == 1:
                ...
                color_value = (percentile - percentiles[-1]) / (
            ...
            for contour in contours:
                plt.plot(
                    contour[:, 1], contour[:, 0], linewidth=linewidth, color=color_value
                )
