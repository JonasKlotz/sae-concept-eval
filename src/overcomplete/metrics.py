...
def r2_score(x, x_hat) -> torch.tensor:
    """
...
def hungarian_loss(dictionary1, dictionary2, p_norm=2):
    ...
    float
        Hungarian loss.
    """
    assert dictionary1.shape == dictionary2.shape, (
...
def energy_of_codes(codes, dictionary):
    ...
    assert codes.shape[1] == dictionary.shape[0], (
        "Number of codes must match dictionary size"
    )
