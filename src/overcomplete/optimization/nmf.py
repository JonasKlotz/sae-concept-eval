...
def multiplicative_update(
    A, Z, D, update_Z=True, update_D=True, max_iter=500, tol=1e-5, verbose=False
):
    """
...
class NMF(BaseOptimDictionaryLearning):
    ...
    def init_nndsvda(self, A):
        ...
        """
        Z, D = _initialize_nmf(
            A.detach().cpu().numpy(), self.nb_concepts, init="nndsvda"
        )
