...
def _one_step_multiplicative_update(
    A, Z, W, update_Z=True, update_W=True, strict_convex=False, epsilon=1e-10
):
    AAt = A @ A.T
    Wt = W.T
...
def cnmf_multiplicative_update_solver(
    ...
    **kwargs,
    ...
    for _ in tqdm(range(max_iter), disable=not verbose):
        Z_old = Z.clone()
        Z, W = _one_step_multiplicative_update(
            A, Z, W, update_Z=update_Z, update_W=update_W, strict_convex=strict_convex
        )
...
def cnmf_pgd_solver(
    ...
    for iter_i in tqdm(range(max_iter), disable=not verbose):
        ...
        D = W @ A
        loss = torch.mean(torch.square(A - (Z @ D))) + l1_penalty * torch.mean(
            torch.abs(Z)
        )
...
class ConvexNMF(BaseOptimDictionaryLearning):
    ...
    def encode(self, A, max_iter=300, tol=None):
        ...
        Z, _ = self.solver_fn(
            ...
            update_Z=True,
            update_W=False,
            strict_convex=self.strict_convex,
            max_iter=max_iter,
            tol=tol,
