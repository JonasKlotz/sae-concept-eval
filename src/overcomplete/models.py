...
class BaseModel(ABC, nn.Module):
    ...
    def encode_image(self, x):
        """
        Encode input images using the DiNoV2 model.
...
class DinoV2(BaseModel):
    ...
    def forward_features(self, x):
        ...
        with torch.no_grad():
            if self.use_half:
                x = x.half()
            f_features = self.model.forward_features(x)
            clstoken = f_features["x_norm_clstoken"]  # [B, N, D]
            return clstoken
            # patch_tokens = rearrange(patch_tokens, "b n d -> (b n) d")
            # print(patch_tokens.shape)
            # print(f_features.keys())
            # # print all shapes of the features
...
class SigLIP(BaseModel):
    ...
    def __init__(self, use_half=False, device="cpu"):
        ...
        self.model = (
            timm.create_model(
                "vit_base_patch16_siglip_224", pretrained=True, num_classes=0
            )
            .eval()
            .to(self.device)
        )
...
class ResNet(BaseModel):
    ...
    def __init__(self, use_half=False, device="cpu"):
        ...
        self.model = (
            timm.create_model("resnetv2_50x1_bit.goog_distilled_in1k", pretrained=True)
            .eval()
            .to(self.device)
        )
        if self.use_half:
...
class ConvNeXt(BaseModel):
    ...
    def __init__(self, use_half=False, device="cpu"):
        ...
        self.model = (
            timm.create_model("convnext_small.fb_in1k", pretrained=True)
            .eval()
            .to(self.device)
        )
        if self.use_half:
