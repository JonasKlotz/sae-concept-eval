...
![Overview of the method](images/full_method.png)
...
- **synCUB** — https://huggingface.co/datasets/jokl/syncub
- **synCOCO** — https://huggingface.co/datasets/jokl/syncoco (`v1.0`, and latest `v2.0`)

Each repo ships `images/`, a `metadata.csv`, and a self-contained PyTorch
`Dataset` loader. Download a specific version and load it:
...
scripts under `src/scripts/hf_export/` (`export_syncub_hf.py`,
`export_syncoco_hf.py`).
...
**Models:** `clip` (ViT-L-14, primary), `dinov2` (ViT-S-14).
**SAE variants:** `topk`, `batchtopk`, `matryoshka`, `jumprelu`, plus `random`
and `frozen` baselines.
...
```bibtex
@inproceedings{klotz2026interpretability,
